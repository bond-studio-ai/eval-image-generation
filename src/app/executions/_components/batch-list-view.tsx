  ...
  return (
    <div className="space-y-6">
      {strategyOrder.map((stratId) => {
        ...
        const maxExec = Math.max(0, ...Array.from(byPreset.values(), (a) => a.length));
              ...
              <table className="divide-border divide-y" style={{ borderCollapse: "separate", borderSpacing: 0 }}>
                ...
                <tbody className="divide-border bg-surface divide-y">
                  {presetNames.map((rowKey) => {
                    ...
                    const rowGenerationIds = presetRuns.map((run) => run.lastOutputGenerationId).filter((id): id is string => !!id);
                    return (
...
function RunCell({
  ...
  function content() {
    if (!run) return <span className="text-text-disabled">&mdash;</span>;

    if (run.lastOutputUrl) {
      return (
