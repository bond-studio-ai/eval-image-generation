              ...
              <table className="divide-border divide-y" style={{ borderCollapse: "separate", borderSpacing: 0 }}>
                <thead className="bg-surface-muted">
                  <tr>
                    ...
                    {Array.from({ length: maxExec }, (_, i) => (
                      <th key={i} className="text-caption text-text-secondary px-2 py-2.5 text-center font-medium tracking-wider uppercase" style={{ width: CELL, minWidth: CELL }}>
                        #{i + 1}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody className="divide-border bg-surface divide-y">
                  {presetNames.map((rowKey) => {
                    ...
                    const rowGenerationIds = presetRuns.map((r) => r.lastOutputGenerationId).filter((id): id is string => !!id);
                    return (
                      <tr key={rowKey} className="hover:bg-surface-muted/50">
                        <td className="border-border bg-surface text-body text-text-primary sticky left-0 z-20 border-r px-4 py-2 font-medium" style={{ minWidth: 200, maxWidth: 200 }}>
                          ...
                          {rowGenerationIds.length > 0 && <ReviewRunGroupBadge generationIds={rowGenerationIds} statuses={segmentationStatuses} setStatus={setSegmentationStatus} />}
                        </td>
                        {Array.from({ length: maxExec }, (_, i) => {
                          ...
                          return (
                            <RunCell
                              ...
                              segmentationState={cellGenerationId ? segmentationStatuses.get(cellGenerationId) : undefined}
                            />
                          );
                        })}
                      </tr>
      ...
      <div className="flex h-full w-full flex-col items-center justify-center gap-1">
        {!run ? (
          ...
          <div className="group relative block">
            <button type="button" onClick={() => onImageClick(run)} className="relative block cursor-pointer">
              <CdnImage
          ...
          <>
            ...
            {(run.status === "failed" || run.status === "skipped") && (
              <button type="button" onClick={() => onRetry(run.id)} disabled={retryingRunId === run.id} className="text-caption text-warning-700 hover:text-warning-600 font-medium disabled:opacity-50">
