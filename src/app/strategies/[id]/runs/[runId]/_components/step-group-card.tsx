    ...
    return (
      <div className="flex flex-col gap-1.5">
        <div className={`relative overflow-hidden rounded-lg border-2 ${isSelected ? "border-amber-400 ring-2 ring-amber-200" : "border-gray-200"}`}>
          <ExpandableImage src={sr.outputUrl} alt={`Generation ${index + 1}`} wrapperClassName="relative block h-48 w-full cursor-pointer bg-gray-50" />
          {isSelected && (
      ...
      <div className="flex flex-col gap-1.5">
        <div className="flex h-48 w-full flex-col items-center justify-center gap-2 rounded-lg border-2 border-red-300 bg-red-50 p-3">
          ...
          <div className="text-center">
            ...
            {sr.error && <p className="mt-1 line-clamp-3 text-[10px] leading-tight text-red-600">{sr.error}</p>}
          </div>
        </div>
        <div className="flex items-center gap-2 text-[11px] text-gray-500">
          <span className="font-medium text-gray-700">{label}</span>
    ...
    return (
      <div className="flex flex-col gap-1.5">
        <div className="flex h-48 w-full flex-col items-center justify-center gap-2 rounded-lg border-2 border-amber-200 bg-amber-50/50 p-3">
          ...
          {sr.error && <p className="line-clamp-2 text-center text-[10px] leading-tight text-amber-500">{sr.error}</p>}
        </div>
        <div className="flex items-center gap-2 text-[11px] text-gray-500">
          <span className="font-medium text-gray-700">{label}</span>
          <StatusBadge status={sr.status} />
...
export function StepGroupCard({
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-xs">
      <button type="button" onClick={() => setOpen(!open)} className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-gray-50">
        ...
        {isMulti && <span className="inline-flex items-center rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-medium text-blue-700">&times;{group.results.length}</span>}
        <span className="text-xs text-gray-500">{group.model}</span>
        {step?.promptVersion && <span className="hidden text-xs text-gray-400 sm:inline">· {step.promptVersion.name || "Untitled prompt"}</span>}
        <span className="ml-auto flex items-center gap-2">
          {stepWallClockMs > 0 && (
            <span className="text-xs text-gray-400 tabular-nums" title={isMulti ? "Longest candidate (parallel)" : "Generation time"}>
      ...
      {open && (
        <div className="border-t border-gray-200">
          <div className="p-4">
            ...
            {(step?.dollhouseViewFromStep || step?.realPhotoFromStep || step?.moodBoardFromStep) && (
              <div className="mb-3 flex flex-wrap gap-2">
                {step.dollhouseViewFromStep && (
                  <span className="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-medium text-amber-700 ring-1 ring-amber-600/20 ring-inset">Dollhouse View &larr; Step {step.dollhouseViewFromStep}</span>
                )}
                {step.realPhotoFromStep && (
                  <span className="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-medium text-amber-700 ring-1 ring-amber-600/20 ring-inset">Real Photo &larr; Step {step.realPhotoFromStep}</span>
                )}
                {step.moodBoardFromStep && (
                  <span className="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-medium text-amber-700 ring-1 ring-amber-600/20 ring-inset">Mood Board &larr; Step {step.moodBoardFromStep}</span>
                )}
              </div>
            )}
            ...
            {isMulti ? (
              <div>
                <p className="mb-2 text-[11px] font-semibold tracking-wider text-gray-400 uppercase">{group.results.length} generations from this step</p>
              ...
              (() => {
                ...
                if (sr.status === "completed" && sr.outputUrl) {
                  return (
                    <div>
                      <ExpandableImage src={sr.outputUrl} alt={`${group.name} output`} wrapperClassName="relative block h-80 w-full max-w-xl rounded-lg border border-gray-200 bg-gray-50" />
                      {sr.generationId && (
