        ...
        {images.map((img, i) => (
          <div key={img.url} className="group relative">
            {img.isComposite ? (
              ...
              <ExpandableImage src={img.url} alt={img.label} wrapperClassName="relative block aspect-square w-full overflow-hidden rounded-md border border-gray-200 bg-gray-50" className="h-full w-full object-cover" />
            )}
        ...
        <div className="rounded-lg border border-violet-200 bg-violet-50 p-3">
          ...
          <div className="grid grid-cols-4 gap-2 sm:grid-cols-6 md:grid-cols-8">
            {images[expandedGroup].sourceImages!.map((src) => (
              <div key={src.url}>
                <ExpandableImage src={src.url} alt={src.label} wrapperClassName="relative block aspect-square w-full overflow-hidden rounded-md border border-violet-200 bg-white" className="h-full w-full object-cover" />
...
export function AuditCollapsible({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="border-t border-gray-100">
      <button type="button" onClick={() => setOpen(!open)} className="flex w-full items-center gap-2 px-4 py-2 text-left text-xs font-medium text-gray-500 hover:bg-gray-50">
  ...
  return (
    <AuditCollapsible title="Audit Details">
      <div className="space-y-3">
        {sr.requestConfig && (
          <div>
            <p className="mb-1 text-[10px] font-semibold tracking-wider text-gray-400 uppercase">Request Config</p>
        ...
        {sr.processedSystemPrompt && (
          <div>
            <p className="mb-1 text-[10px] font-semibold tracking-wider text-gray-400 uppercase">System Prompt</p>
            <pre className="max-h-48 overflow-auto rounded-md border border-gray-200 bg-gray-50 p-2 text-xs leading-relaxed whitespace-pre-wrap text-gray-700">{sr.processedSystemPrompt}</pre>
          </div>
        )}
        {sr.processedUserPrompt && (
          <div>
            <p className="mb-1 text-[10px] font-semibold tracking-wider text-gray-400 uppercase">User Prompt</p>
            <pre className="max-h-48 overflow-auto rounded-md border border-gray-200 bg-gray-50 p-2 text-xs leading-relaxed whitespace-pre-wrap text-gray-700">{sr.processedUserPrompt}</pre>
          </div>
        )}
        {sr.inputImages && sr.inputImages.length > 0 && (
          <div>
            <p className="mb-1 text-[10px] font-semibold tracking-wider text-gray-400 uppercase">Input Images ({sr.inputImages.length})</p>
