...
}: ProjectDataSectionProps) {
  const includedCount = bootstrap ? bootstrap.cameraFrames.length - excludedFrameKeys.size : 0;

  return (
    <FormSection
      title="Project data"
      description="Review the project's camera frames and override design materials or room data if needed."
    >
      {!bootstrap ? (
        ...
        <div className="min-w-0">
          ...
          {(bootstrap.project.name || bootstrap.project.appStatus) && (
            <p className="text-caption text-text-muted mt-0.5 truncate">
              {[bootstrap.project.name, bootstrap.project.appStatus].filter(Boolean).join(' · ')}
            </p>
          )}
        </div>
...
function OverridePanel({
  ...
  const summary = (() => {
    const bits = [
      designResult.provided ? 'design materials' : null,
      roomResult.provided ? 'room data' : null,
    ].filter(Boolean);
  ...
  return (
    <div className="border-border-subtle mt-6 border-t pt-6">
      ...
      >
        <div className="min-w-0">
          <h3 className="text-body text-text-primary font-semibold">
            Override project data (optional)
          </h3>
      ...
      {isOpen && (
        <div className="mt-4 space-y-5">
          <Field
            label="Design materials (Unity-slim JSON)"
            optional
            hint={
              hasBootstrap && !designResult.provided
                ? 'Empty — using the value loaded from the project.'
                : undefined
            }
            error={designResult.error ?? undefined}
          >
            {(id) => (
              <Textarea
                id={id}
                value={designMaterialsInput}
                onChange={(e) => setDesignMaterialsInput(e.target.value)}
          ...
          <Field
            ...
            optional
            hint={
              hasBootstrap && !roomResult.provided
                ? 'Empty — using the value loaded from the project.'
                : undefined
            }
            error={roomResult.error ?? undefined}
          >
            {(id) => (
              <Textarea
                id={id}
                value={roomDataInput}
                onChange={(e) => setRoomDataInput(e.target.value)}
