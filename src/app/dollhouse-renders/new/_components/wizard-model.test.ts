...
describe("buildWizardModel — per-step issues are scoped", () => {
  ...
  it("reports missing project materials only in the data step", () => {
    const model = buildWizardModel(input({ bootstrap: bootstrap({ designMaterials: null, roomData: null }) }));
    expect(model.dataIssues).toEqual(expect.arrayContaining(["Design materials are missing for this project.", "Room layout is missing for this project."]));
    expect(model.projectIssues).toEqual([]);
  });
