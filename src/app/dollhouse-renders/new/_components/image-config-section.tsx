...
export function ImageConfigSection({ value, onChange }: { value: ImageConfigState; onChange: (next: ImageConfigState) => void }) {
  return (
    <FormSection title="Image config" description="Output image dimensions and format.">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="md:col-span-2">
          ...
          <SegmentedControl options={FORMAT_OPTIONS} value={value.format} onChange={(format) => onChange({ ...value, format })} label="Output image format" />
        </div>
        <NumberInput label="Width" value={value.width} onChange={(width) => onChange({ ...value, width })} min={1} />
        <NumberInput label="Height" value={value.height} onChange={(height) => onChange({ ...value, height })} min={1} />
        <NumberInput
