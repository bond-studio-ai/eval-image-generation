...
export function StrategyPerformanceSection({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await fetch(imageGenerationApiUrl(`analytics/strategy-performance?${params}`), { cache: 'no-store' });
  ...
  const fetchBreakdown = useCallback(async (strategyId: string) => {
    ...
    try {
      const res = await fetch(imageGenerationApiUrl(`analytics/strategy-errors?strategy_id=${encodeURIComponent(strategyId)}`), { cache: 'no-store' });
      if (!res.ok) return;
      const json = await res.json();
