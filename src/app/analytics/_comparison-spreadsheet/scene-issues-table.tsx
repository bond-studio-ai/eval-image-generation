    ...
    <div className="overflow-x-auto rounded-lg border border-gray-300 bg-white shadow-xs">
      <table className="w-full border-collapse text-xs">
        <thead>
          ...
          <tr>
            <th aria-label="Issue" className="w-48 min-w-[180px] border-r border-b border-gray-300 bg-white px-3 py-2" />
            {slices.map((slice, i) => {
              const color = SLICE_BG_COLORS[i % SLICE_BG_COLORS.length];
              return (
                <th key={slice.key} className={`border-r border-b border-gray-300 px-3 py-2.5 text-center ${color.header}`} style={{ minWidth: 160 }}>
          ...
          <tr className="bg-gray-50/60">
            <th className="border-r border-b border-gray-200 px-3 py-1.5 text-left text-[11px] font-semibold text-gray-700">Overall</th>
            {slices.map((slice, i) => {
              const s = dataBySlice[slice.key]?.summary;
              const color = SLICE_BG_COLORS[i % SLICE_BG_COLORS.length];
              return (
                <td key={slice.key} className={`border-r border-b border-gray-200 px-2 py-1.5 text-center text-[11px] ${color.header}`}>
                  {s ? (
                    <>
          ...
          <tr className="bg-gray-100">
            <th className="sticky left-0 z-10 border-r border-b border-gray-300 bg-gray-100 px-3 py-2 text-left text-[10px] font-bold tracking-wider text-gray-600 uppercase">Issue</th>
            {slices.map((slice) => (
          ...
          {!loading &&
            sceneIssueRows.map((issueName) => (
              <tr key={issueName} className="bg-white">
                <th className="sticky left-0 z-10 border-r border-b border-gray-200 bg-white px-3 py-1.5 text-left text-[11px] font-normal text-gray-700">{issueName}</th>
                {slices.map((slice) => {
                  ...
                  return (
                    <td key={slice.key} className="border-r border-b border-gray-200 px-2 py-1.5 text-center text-[11px] text-red-600">
                      {item ? `${item.count} (${pct}%)` : "-"}
                    </td>
                  );
                })}
