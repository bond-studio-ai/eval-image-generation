...
export function StrategyTableHeader({ sortKey, sortDir, onSort }: { sortKey: SortKey; sortDir: SortDir; onSort: (key: SortKey) => void }) {
  return (
    <thead>
      <tr>
        <th aria-label="Expand row" className="py-3 pr-4 text-left text-xs font-medium tracking-wider text-gray-600 uppercase" style={{ width: 40 }} />
        <th className="cursor-pointer py-3 pr-6 text-left text-xs font-medium tracking-wider text-gray-600 uppercase transition-colors select-none hover:text-gray-900" onClick={() => onSort("name")}>
          Strategy
