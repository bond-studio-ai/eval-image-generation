...
const VALID_CATEGORY_SEGMENTS = new Set(PRODUCT_CATEGORIES.map((k) => k.replaceAll("_", "-")));

const TILE_SNAKE = new Set(["floor_tiles", "wall_tiles", "shower_wall_tiles", "shower_floor_tiles", "shower_curb_tiles"] as const satisfies readonly (typeof PRODUCT_CATEGORIES)[number][]);

const TILE_SEGMENTS = new Set(Array.from(TILE_SNAKE, (k) => k.replaceAll("_", "-")));
...
function extractProductRecords(json: unknown): Record<string, unknown>[] {
  ...
  const node: unknown = root.data ?? root;

  if (Array.isArray(node)) {
    return node.filter((entry): entry is Record<string, unknown> => Boolean(entry) && typeof entry === "object");
  }

  if (node && typeof node === "object" && !Array.isArray(node)) {
    const catalogNode = node as RawCatalogNode;
    if (Array.isArray(catalogNode.data)) {
      return catalogNode.data.filter((entry): entry is Record<string, unknown> => Boolean(entry) && typeof entry === "object");
    }
    for (const key of ["items", "products", "results"] as const) {
      const value = catalogNode[key];
...
const SKIP_KEYS = new Set(["images", "preferredRetailer", "variants"]);
...
export async function GET(_request: Request, { params }: { params: Promise<{ category: string }> }) {
  try {
    ...
    const segment = decodeURIComponent(category).replaceAll("_", "-");
