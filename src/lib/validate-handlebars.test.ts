...
describe('validateHandlebarsTemplate', () => {
  ...
  it('reports unclosed expressions', () => {
    expect(validateHandlebarsTemplate('Hello {{name')).toEqual([
      { line: 1, message: 'Unclosed "{{" — missing closing "}}"' },
    ]);
  });

  it('reports orphan close tags', () => {
    expect(validateHandlebarsTemplate('{{/if}}')).toEqual([
      { line: 1, message: 'Unexpected {{/if}} with no matching {{#if}}' },
    ]);
  });
