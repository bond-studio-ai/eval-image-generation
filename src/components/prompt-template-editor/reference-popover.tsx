          ...
          {!state.category ? (
            <>
              ...
              <div className="max-h-60 overflow-auto py-1">
                {options.map((cat) => (
                  <button key={cat.value} type="button" onClick={() => onCategorySelect(cat)} className="w-full px-3 py-2 text-left text-sm text-gray-900 hover:bg-gray-50">
                    {cat.label}
                  </button>
                ))}
            ...
            <>
              <div className="flex items-center gap-1 border-b border-gray-100 px-2 py-1.5">
                ...
                <span className="min-w-0 flex-1 truncate text-xs font-medium text-gray-600">{REFERENCE_OPTIONS.find((o) => o.value === state.category)?.label ?? state.category}</span>
              </div>
              <p className="border-b border-gray-50 px-3 py-1.5 text-[11px] text-gray-500">
                Pick a field to insert <code className="rounded bg-gray-100 px-0.5">{`{{products.${REFERENCE_OPTIONS.find((o) => o.value === state.category)?.singular ?? state.category}.…}}`}</code>
              </p>
              {attrState.loading ? (
                ...
                <div className="max-h-60 overflow-auto py-1">
                  {attrState.list.map((attr) => {
                    ...
                    return (
                      <button key={attr} type="button" onClick={() => onAttributeSelect(attr, singular)} className="w-full px-3 py-2 text-left font-mono text-xs text-gray-900 hover:bg-gray-50">
                        {attr}
                      </button>
                    );
                  })}
