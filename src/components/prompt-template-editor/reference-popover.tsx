          ...
          {!state.category ? (
            <>
              ...
              <div className="max-h-60 overflow-auto py-1">
                {options.map((cat) => (
                  <button key={cat.value} type="button" onClick={() => onCategorySelect(cat)} className="text-text-primary hover:bg-surface-muted text-body w-full px-3 py-2 text-left">
                    {cat.label}
                  </button>
                ))}
            ...
            <>
              <div className="border-border-subtle flex items-center gap-1 border-b px-2 py-1.5">
                ...
                <span className="text-text-secondary text-caption min-w-0 flex-1 truncate font-medium">{REFERENCE_OPTIONS.find((o) => o.value === state.category)?.label ?? state.category}</span>
              </div>
              <p className="border-border-subtle text-text-muted border-b px-3 py-1.5 text-[11px]">
                Pick a field to insert <code className="bg-surface-sunken rounded px-0.5">{`{{products.${REFERENCE_OPTIONS.find((o) => o.value === state.category)?.singular ?? state.category}.…}}`}</code>
              </p>
              {attrState.loading ? (
                ...
                <div className="max-h-60 overflow-auto py-1">
                  {attrState.list.map((attr) => {
                    ...
                    return (
                      <button key={attr} type="button" onClick={() => onAttributeSelect(attr, singular)} className="text-text-primary hover:bg-surface-muted text-caption w-full px-3 py-2 text-left font-mono">
                        {attr}
                      </button>
                    );
                  })}
