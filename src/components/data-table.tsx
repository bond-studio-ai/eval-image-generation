...
export const TD_DEFAULT = 'whitespace-nowrap px-6 py-4 text-sm text-gray-700';
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {loading ? (
              ...
              data.map((row) => (
                <tr key={rowKey(row)} className={rowClassName?.(row) ?? 'hover:bg-gray-50'}>
                  {columns.map((col, i) => (
...
export function NameCell({
  ...
  return (
    <div>
      <Link href={href} className="text-primary-600 hover:text-primary-500 text-sm font-medium">
        {name || fallback}
      </Link>
      {subtitle && <p className="mt-0.5 max-w-xs truncate text-xs text-gray-500">{subtitle}</p>}
    </div>
  );
}
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        className="focus:border-primary-500 focus:ring-primary-500 w-full rounded-lg border border-gray-300 py-1.5 pr-3 pl-9 text-sm text-gray-900 placeholder:text-gray-400 focus:ring-1 focus:outline-none"
      />
    </div>
  );
}
  ...
  return (
    <label className="flex cursor-pointer items-center gap-2 select-none">
      <button
        ...
        className={`focus-visible:ring-primary-500 relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none ${
...
const VARIANT_CLASSES = {
  default:
    'rounded p-1.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 disabled:opacity-50',
  danger:
    'rounded p-1.5 text-gray-400 transition-colors hover:bg-red-50 hover:text-red-600 disabled:opacity-50',
};
