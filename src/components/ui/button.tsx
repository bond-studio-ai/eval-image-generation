...
const BASE = "inline-flex items-center justify-center rounded-button font-medium transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 disabled:cursor-not-allowed";
...
export interface ButtonProps extends ButtonOwnProps, Omit<ButtonHTMLAttributes<HTMLButtonElement>, "children"> {
  children?: ReactNode;
  ref?: Ref<HTMLButtonElement>;
}

export function Button({ variant = "primary", size = "md", loading = false, iconLeft, iconRight, fullWidth = false, disabled, className, children, type = "button", ref, ...rest }: ButtonProps) {
...
export function LinkButton({ href, variant = "primary", size = "md", iconLeft, iconRight, fullWidth = false, external = false, target, rel, className, children }: LinkButtonProps) {
  const linkSizing = variant === "link" ? "" : SIZE[size];
