    ...
    <div className="border-border bg-surface-muted rounded-md border p-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex min-w-0 items-center gap-1.5">
          <span className="ring-border-strong inline-block size-3 shrink-0 rounded-sm ring-1" style={{ backgroundColor: swatch }} aria-hidden="true" />
          <p className="text-text-secondary text-caption truncate font-semibold" title={row.label}>
            {row.label}
          </p>
        </div>
        {row.topScore !== null && <span className="bg-surface text-text-secondary ring-border shrink-0 rounded px-1.5 py-0.5 text-[10px] tabular-nums ring-1">{row.topScore.toFixed(2)}</span>}
      </div>
      {promptLine && (
        <p className="text-text-secondary mt-0.5 truncate text-[10px]" title={promptLine}>
          {promptLine}
        </p>
      )}
      ...
      {consumerHint && (
        <p className="text-text-disabled mt-0.5 truncate text-[10px]" title={consumerHint}>
          {consumerHint}
        </p>
      )}

      {row.masks.length > 0 ? (
        <div className="border-border bg-surface relative mt-2 block aspect-square w-full overflow-hidden rounded border">
        ...
        <div className="border-border bg-surface mt-2 flex aspect-square w-full items-center justify-center rounded border border-dashed">
          <p className="text-text-disabled px-2 text-center text-[10px] italic">No masks returned</p>
        </div>
      )}

      {showIndividualMasks && (
        <div className="mt-2">
          <p className="text-text-muted mb-1 text-[10px] font-semibold tracking-wide uppercase">Individual masks</p>
          <div className="grid grid-cols-3 gap-1.5">
            {row.masks.map((mask, idx) => (
              ...
              >
                ...
                {mask.score !== null && <span className="text-text-inverse bg-overlay/60 absolute right-0.5 bottom-0.5 rounded px-1 py-px text-[9px] font-medium tabular-nums">{mask.score.toFixed(2)}</span>}
              </a>
            ))}
          </div>
        </div>
...
export function SegmentationLegend({ rows }: { rows: CategoryRow[] }) {
  ...
  return (
    <div className="border-border bg-surface-muted mt-3 rounded-md border px-3 py-2">
      <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
        {entries.map(([category, entry]) => (
          <div key={category} className="flex min-w-0 items-center gap-1.5">
            <span className="ring-border-strong inline-block size-3 shrink-0 rounded-sm ring-1" style={{ backgroundColor: entry.color }} aria-hidden="true" />
            <span className="text-text-secondary truncate text-[11px] leading-tight" title={`${entry.displayLabel} · ${entry.totalMasks} ${entry.totalMasks === 1 ? "mask" : "masks"}`}>
              {entry.displayLabel}
            </span>
          </div>
        ))}
    ...
    <div className="mb-5">
      ...
      >
        <span className="flex items-center gap-2">
          ...
          <span className="text-text-secondary text-caption font-semibold tracking-wide uppercase">Per-category masks</span>
