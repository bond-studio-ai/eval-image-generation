...
export function Tooltip({ children, hint, width, triggerClassName, align }: { children: ReactNode; hint: ReactNode; width?: number; triggerClassName?: string; align?: TooltipAlign }) {
  ...
  return (
    <>
      <button ref={ref} type="button" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} onFocus={onFocus} onBlur={onBlur} className={`inline-flex cursor-help outline-none ${triggerClassName ?? ""}`}>
        {children}
      </button>
      {portal}
    </>
