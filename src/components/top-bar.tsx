...
const SEGMENT_LABELS: Record<string, string> = {
  ...
  'catalog-thresholds': 'Thresholds',
  'judge-baselines': 'Judge Baselines',
...
export function TopBar({ className }: TopBarProps) {
  ...
  return (
    <div
      className={cn(
        'border-border bg-surface/80 sticky top-0 z-20 flex h-12 shrink-0 items-center justify-between gap-4 border-b px-4 backdrop-blur-sm sm:px-6',
        className,
      )}
    >
            ...
            return (
              <li key={`${crumb.label}-${i}`} className="flex shrink-0 items-center gap-1">
                ...
                {crumb.href && !isLast ? (
                  <Link
                    href={crumb.href}
                    className="text-text-muted hover:text-text-primary truncate"
                  >
                    {crumb.label}
                  </Link>
                ) : (
      ...
      <div className="flex shrink-0 items-center gap-3">
        {env && env !== 'production' && (
          <span className="rounded-pill bg-warning-50 text-warning-800 ring-warning-600/30 inline-flex items-center px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset">
            {env.toUpperCase()}
          </span>
        )}
        <UserButton
